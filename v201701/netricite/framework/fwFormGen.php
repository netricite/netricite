...
class fwFormGen{
    ...
    private $data;   // data to be displayed in the form
    private $errors; // errors given by validator
    private $lang; // errors given by validator

    /**
     * constructor
     *
     * @param array $data data to be displayed in the form
     */
    public function __construct($data=null)
    {
        ...
        $this->setData($data);
        $this->setErrors(null);
        //$this->lang= fw\fwConfigLanguage::getConfig();
    }
    
    /**
     * initialize BS form with data
     * 
     * @param array $data list of fields
     */
     ...
     * 
     * @param array $field list of fields in the form
     * @return string 
     */
      ...
      * BS Form
      *
      * @param string $formclass : BS class
      * @param int $col : #column
      * @param array $param : attributes of the form
     ...
     function formbegin($attributes=array(), $request=array() , $record=array()){
         ...
         $html="<form ".$formclass." name='$name' role='form' action='index.php?application=". $application 
            ."&class=".$class.$urlparam. "' method='post' enctype='multipart/form-data'>";
      ...
      * content of a field
      *
      * @param string $field index of the field in the data array
      * @return string value
      */
      ...
      * @param string $col  #bootstrap colums
     ...
     function button($attributes = array()){
         ...
         $class =(!empty($class)) ? $class : "btn btn-default btn-sm pull-right";
     ...
     function buttonSession($attributes = array()){
         ...
         if (!empty($_SESSION['pseudo'])) {
             $html .= $this->button(array("field"=>$field,"col"=>$col,"icon"=>$icon,"text"=>$this->translate($field,"text", $text), 
                 "value"=>$value, "tooltip"=>$tooltip, "attr"=>$attr));
         } else {
             $html .= $this->button(array("field"=>$field,"col"=>$col,"icon"=>"ban-circle","class"=>"pull-right","text"=>$this->translate($field,"text", $text), 
      ...
      * @param string $field
      * @param string $label
      * @param string $option select option value=>associated name
      * @return string html statement
      */
      ...
      * @param string $field
      * @param string $label
      * @param string $option checkbox list value=>associated name
      * @return string html statement
      */
     ...
     function image($attributes=array()){
         ...
         fwWatch(array("imgfilePath"=>$imgfilePath,"imgsrcPath"=>$imgsrcPath,"img"=>$file),"", get_class($this));
     ...
     /*
     function image($class="responsive img-medium", $file, $style=""){
         $html = '<img class="'.$class.'" src="public/img/leTraineau/'.$file.'" alt="'.$file.'" '.$style.' />';
         return $html;
     }
     */
     /**
     ...
     function fit_box($filename, $box = 200)
     {
         ...
         if (!is_readable($filename)) throw new \Exception("fwFormGen.fit_box(image file not found) " . $filename );
      ...
      *
      * @param url $index url of the link
     ...
     function imageLink($attributes=array()){
         ...
         $html .= $this->image(array("class"=>"img-pic","file"=>$file));
     ...
     function imageCart($attributes = array()){
         fwTrace(debug_backtrace(),$attributes);
         extract($attributes);
         $html=$this->imageThumbnailBegin($attributes);
         $attributes["legend"]= $this->iconHref('index.php?application=shop&class=cart&action=add&id=' .$id, "shopping-cart", "Ajouter au panier", $legend );
         $html.=$this->imageThumbnailEnd($attributes);
         return $html;
     }
    /*
     function imageCart($col="", $class="responsive", $file,  $id, $title="", $legend=""){
         $html = $this->divbegin($col);
         $html .= '<div class="thumbnail">';
         $class=($class!="") ? $class:"responsive img-thumbnail";
         $html .= $this->image($class, $file, 'style="width:304px;height:228px;"');
         $legend= $this->iconHref('index.php?application=shop&class=cart&action=add&id="' .$id, "shopping-cart", "Ajouter au panier", $legend );
         $html .= $this->caption($title, $legend) ;
         $html .= $this->divend("1") . $this->rowend();
         return $html;
     }
    */
